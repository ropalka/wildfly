...
public class HttpListenerService extends ListenerService {
    ...
    public HttpListenerService(Consumer<ListenerService> serviceConsumer, String name, final String serverName, OptionMap listenerOptions, OptionMap socketOptions, boolean certificateForwarding, boolean proxyAddressForwarding, boolean proxyProtocol) {
        super(serviceConsumer, name, listenerOptions, socketOptions, proxyProtocol);
    ...
    protected void preStart(final StartContext context) {
        ...
        final ServiceBuilder<?> sb = context.getChildTarget().addService(HTTP_UPGRADE_REGISTRY.append(getName()));
        final Consumer<Object> serviceConsumer = sb.provides(HTTP_UPGRADE_REGISTRY.append(getName()));
